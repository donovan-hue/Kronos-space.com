...
| AUDIT-003-405 | Login desde `kronos-space.com` fallaba con 405 | **ARREGLADO Y DESPLEGADO** | Cloudflare Pages no proxea `/api` y responde 405 a los POST. `client/src/services/apiUrl.js` resuelve la base a `https://api.kronos-space.com/api` en hosts estáticos y deja `/api` solo en desarrollo; 5 pruebas nuevas en `client/test/apiUrl.test.mjs`. `_redirects` descartado: Cloudflare no proxea dominios externos. Producción reconstruida tras el merge de #7 y verificada en el commit `dbbc656` |
| AUDIT-003-VAL | Bundle desplegado apuntando a la API | **VALIDADO EN PRODUCCIÓN** | 2026-09-16T20:26Z, informe del CI en el commit `dbbc656`: `kronos-space.com` sirve `/assets/index-bgh-vUsX.js` con `https://api.kronos-space.com/api` y **sin URLs de localhost**; la app de Vercel igual. Antes: `index-DxciSEbr.js` con `http://localhost:5000/api` |
