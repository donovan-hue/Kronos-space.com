...
function AppContent() { const [user, setUser] = useState(getUser); useEffect(() => { const interceptor = api.interceptors.response.use((response) => response, (error) => { if (error.response?.status === 401) { clearSession(); disconnectSocket(); setUser(null); } return Promise.reject(error); }); return () => api.interceptors.response.eject(interceptor); }, []); useEffect(() => { const token = getToken(); if (!token) return undefined; let active = true; api.get("/auth/me").then(({ data }) => { if (active && data.user) setUser(data.user); }).catch(() => { if (active) { clearSession(); setUser(null); } }); return () => { active = false; }; }, []); useEffect(() => { const token = getToken(); if (!user || !token) { disconnectSocket(); return undefined; } connectSocket(token); return () => disconnectSocket(); }, [user]); function logout() { clearSession(); setUser(null); } return <Routes><Route path="/login" element={user ? <Navigate replace to="/home" /> : <Auth onLogin={setUser} initialMode="login" />} /><Route path="/register" element={user ? <Navigate replace to="/home" /> : <Auth onLogin={setUser} initialMode="register" />} />
