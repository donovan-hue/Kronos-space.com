          ...
          <div className="k-feed-list">
            {posts.map((post) => (
              <article className="k-post" key={post._id}>
                ...
                {post.media?.url && <div style={{ margin: "0 20px 12px", overflow: "hidden", borderRadius: 10, border: "1px solid var(--k-border)" }}><img src={post.media.url} alt={post.media.alt || ""} loading="lazy" style={{ width: "100%", maxHeight: 420, objectFit: "cover" }} /></div>}
