...
export default function ProfileSettings() {
  ...
  useEffect(() => {
    api.get("/users/me")
      .then(({ data }) => setForm({ displayName: data.displayName || "", bio: data.bio || "", avatar: data.avatar || "" }))
  ...
  async function handleFile(event) {
    ...
    try {
      ...
      setForm({ displayName: updated.displayName || "", bio: updated.bio || "", avatar: updated.avatar || "" });
  ...
  async function save(event) {
    ...
    try {
      ...
      setForm({ displayName: data.displayName || "", bio: data.bio || "", avatar: data.avatar || "" });
      ...
      <form className="k-ai-form k-surface" onSubmit={save}>
        ...
        {form.avatar && (
          <div style={{ overflow: "hidden", borderRadius: 12, border: "1px solid var(--k-border)", maxWidth: 240 }}>
            <img src={form.avatar} alt="Preview avatar" style={{ width: "100%", display: "block" }} />
          </div>
        )}
