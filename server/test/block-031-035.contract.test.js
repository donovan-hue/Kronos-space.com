...
test("031: el modelo de publicaciones conserva un catálogo de reacciones múltiples", () => {
  ...
  assert.deepEqual(Post.schema.path("audience").schema.path("type").enumValues, ["public", "followers", "private"]);
  assert.ok(Post.schema.path("hashtags"));
  assert.deepEqual(AUDIENCE_TYPES, ["public", "followers", "private"]);
});
